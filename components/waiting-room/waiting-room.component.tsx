...
function formatTimeLimit(seconds: number | null): string {
  ...
  if (seconds < 3600)
    return `${Math.floor(seconds / 60)} min ${seconds % 60 > 0 ? `${seconds % 60}s` : ""}`.trim();
...
const WaitingRoom = ({
  ...
  async function handleSaveName() {
    ...
    try {
      const updated = await updatePlayerNameApi(
        currentGame.id,
        userId,
        trimmed,
      );
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={styles.nameEditRow}>
            ...
            <Pressable
              style={({ pressed }) => [
                ...
                pressed && styles.nameSaveButtonPressed,
              ]}
      ...
      <View style={settingsStyles.container}>
        ...
        {settingsLine(
          "Joueurs max",
          String(currentGame.settings?.maxPlayers ?? "?"),
        )}
        {settingsLine(
          "Crédits",
          String(currentGame.settings?.maxCredits ?? "?"),
        )}
        {settingsLine(
          "Durée",
          formatTimeLimit(currentGame.settings?.timeLimitSeconds ?? null),
        )}
        {settingsLine(
          "Voir crédits des autres",
          currentGame.settings?.showOtherCredits ? "Oui" : "Non",
        )}
        {settingsLine(
          "Voir scores des autres",
          currentGame.settings?.showOtherScores ? "Oui" : "Non",
        )}
        {settingsLine(
          "Voir qui est Last",
          currentGame.settings?.showOtherIsLast ? "Oui" : "Non",
        )}
      </View>
